...
\pagebreak Note that when using the Hypergeometric distribution, the
default values are \code{N=100} and \code{pd=(0:100)/100} --- when
changing the value of \code{N} you should also explicitly specify
\code{pd} to ensure that \code{N*pd} contains only integer
values.\footnote{Not changing \code{pd} can result in some unexpected
  results --- these come directly from invoking the function
  \code{phyper}, which is part of \proglang{R}.  This is because
  \code{pd*N} should result in integer values, \ie the number of
...
A software package 
can always be improved.  The following is a short list, in no specific order,
of areas which are currently on the to-do list.
\begin{itemize}
